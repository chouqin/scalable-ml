...
def parsePoint(line):
    ...
    """
    items = line.split(',')
    return LabeledPoint(items[0], items[1:])
...
def calcRMSE(labelsAndPreds):
    ...
    return sqrt(labelsAndPreds.map(lambda t: squaredError(t[0], t[1])).mean())
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        gradient = trainData.map(lambda p: gradientSummand(w, p)).sum()
...
def twoWayInteractions(lp):
    ...
    return LabeledPoint(lp.label, np.hstack((lp.features, 
                                            map(lambda t: t[0] * t[1], itertools.product(lp.features, lp.features)))))
