# Databricks notebook source exported at Sat, 25 Jul 2015 02:49:42 UTC
# MAGIC %md
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    return SparseVector(numOHEFeats, [(OHEDict[v], 1) for v in rawFeats])
...
# TODO: Replace <FILL IN> with appropriate code
sampleOHEData = sampleDataRDD.map(lambda d: oneHotEncoding(d, sampleOHEDictManual, numSampleOHEFeats))
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    items = point.split(',')
    label = int(items[0])
    return LabeledPoint(label, oneHotEncoding(enumerate(items[1:]), OHEDict, numOHEFeats))
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    Returns:
        SparseVector: A SparseVector of length numOHEFeats with indicies equal to the unique
            ...
            with values equal to 1.0.
    """
    return SparseVector(numOHEFeats, [(OHEDict[v], 1) for v in rawFeats if v in OHEDict])
...
def parseHashPoint(point, numBuckets):
    ...
    items = point.split(',')
    label = int(items[0])
    return LabeledPoint(label, SparseVector(numBuckets, hashFunction(numBuckets, enumerate(items[1:]))))
